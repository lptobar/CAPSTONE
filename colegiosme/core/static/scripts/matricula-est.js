...
rut.addEventListener('blur', async (ev) => {
    ...
    try {
        const request = await fetch(`/obtener_persona/${run}/`);
...
formulario.addEventListener('submit', async (ev) => {
    ...
    try {
        ...
        const request = await fetch('/crear-matricula/', { headers: { 'Content-Type': 'application/json', 'X-CSRFToken': token }, method: 'POST', body: body });
        const response = await request.json();

        if (response.ok) {
            showMessage('Redirigiendo...');
            setTimeout(() => {
                location.href = `/matricula-padres/${response.id_matricula}/`
            }, 3000);
        } else {
